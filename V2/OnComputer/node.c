...
void setParent(Node *copy, Node orig) {
    if(copy->parent == NULL) {
      copy->parent = malloc(sizeof(Node) + sizeof(Node*));
    }
...
void calculateNodeValues(Node parent, Node goal, int possibleX, int possibleY, char grid[][size.width], Node open[][size.width], int* openLength) {
	...
	hxDiff = abs(goal.x - openNode.x);
	hyDiff = abs(goal.y - openNode.y);
	hCost = hxDiff + hyDiff;
	openNode.h = hCost;
...
void info(Node node) {
    if(node.parent != NULL) {
        fprintf(stderr, "info.. %d, %d. f: %d. id: %d. E: %d ", node.x, node.y, node.f, node.id, node.exists);
        fprintf(stderr, "//// Parent: %d, %d. f: %d. id: %d. E: %d\n", node.parent->x, node.parent->y, node.parent->f, node.parent->id, node.parent->exists);
    }
    else {
        fprintf(stderr, "info.. %d, %d. f: %d. id: %d. E: %d //// Parent = NULL\n", node.x, node.y, node.f, node.id, node.exists);
    }
}
