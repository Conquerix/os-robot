...
CORO_CONTEXT( handle_motor_positions );
...
CORO_DEFINE( handle_sonar_measures ) {
  ...
  CORO_LOCAL char application_message[16];
  ...
  for(;;){
    ...
    fprintf(stderr, "Sonar distance = %f\n", us_distance);
...
CORO_DEFINE( handle_gyro_measures ) {
  ...
  CORO_LOCAL char application_message[6];
...
CORO_DEFINE( handle_motor_positions ){

  CORO_LOCAL char application_message[16], application_message_left[6], application_message_right[6], application_message_sonar[6];
  CORO_LOCAL int left_wheel_pos_old, right_wheel_pos_old, sonar_direction_old;
...
CORO_DEFINE( drive ){
  ...
  for (;;){
    CORO_WAIT((motor_state.left != wheel_command.left) || (motor_state.right != wheel_command.right));
	   ...
	   tacho_set_speed_sp( MOTOR_LEFT, wheel_command.left * max_speed );
	   tacho_set_speed_sp( MOTOR_RIGHT, wheel_command.right * max_speed );
...
int main( int argc, char *argv[] ) {
    ...
    while ( alive ) {
      ...
      CORO_CALL( handle_motor_positions );
