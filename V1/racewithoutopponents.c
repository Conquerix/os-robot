...
void map_middle_left(){
  ...
  if (position_in_circuit != position_in_circuit_old){
    ratio = 60;
    if (compass_direction > 169  && compass_direction < 349){
      command = LEFT;
    } else if (compass_direction < 161  || compass_direction > 349){
      command = RIGHT;
    } else {
      command = FORTH;
      position_in_circuit_old = position_in_circuit;
    ...
    ratio = 60;
    if (compass_direction < 161){
      command = RIGHT;
    } else if (compass_direction > 169){
      command = LEFT;
    } else {
      command = FORTH;
...
void map_bottom_right(){
  if (compass_direction < 4  || compass_direction > 356){
