...
float us_distance_to_middle_wall = 800;
...
void map_middle_right(){
  if (position_in_circuit != position_in_circuit_old){
    ratio = 40;
    if (compass_direction > 3  && compass_direction < 180){
      command = LEFT;
    } else if (compass_direction >= 180  && compass_direction < 357){
      command = RIGHT;
    } else {
      command = FORTH;
...
void map_up_left(){
  ...
  } else if (compass_direction > 177 && compass_direction < 183){
...
void map_bottom_right(){
  ...
  } else if (compass_measure < 6  || compass_measure > 359){
