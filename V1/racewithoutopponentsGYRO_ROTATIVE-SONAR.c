...
#include "coroutine.h"
#include "brick.h"
#define SPEED_LINEAR    65  /* Motor speed for linear motion, in percents */
...
void map_up_right(){
  if (compass_direction > 266 && compass_direction < 274){
...
void map_up_left(){
  ...
  } else if (compass_direction > 176 && compass_direction < 184){
...
void map_middle_left(){
  ...
  if (position_in_circuit != position_in_circuit_old){
    ratio = 60;
    if (compass_direction > 174  && compass_direction < 349){
      command = LEFT;
    } else if (compass_direction < 166  || compass_direction > 349){
      command = RIGHT;
    } else {
      command = FORTH;
      position_in_circuit_old = position_in_circuit;
    ...
    ratio = 60;
    if (compass_direction < 176){
      command = RIGHT;
    } else if (compass_direction > 184){
      command = LEFT;
    } else {
      command = FORTH;
...
void map_bottom_left(){
  ...
  } else if (compass_direction > 86 && compass_direction < 94){
...
void map_bottom_right(){
  ...
  } else if (compass_measure < 13  || compass_measure > 4){
    circuit_direction = compass_measure_uncomputed;
...
CORO_DEFINE( sonar_drive ) {
    ...
    for ( ; ; ) {
        ...
        case STOP:
            ...
            break;
        case LEFT:
            tacho_set_position_sp( MOTOR_SONAR, 2000 );
            tacho_run_to_rel_pos(MOTOR_SONAR);
            printf("Test D");
            break;
        case RIGHT:
            tacho_set_position_sp( MOTOR_SONAR, -2000 );
